...
class CategoryController extends BaseController
{
    ...
    public function store(CategoryRequest $request, BaseHttpResponse $response)
    {
        ...
        $slug=Slug::where(['reference_type'=>'Botble\Blog\Models\Category','reference_id'=>$category->id])->first();
        $string = str_replace(' ', '-', $category->name); // Replaces all spaces with hyphens.

        $slug->key= preg_replace('/[^\x{0600}-\x{06FF}A-Za-z\- %^&*()]/u','', $string);//preg_replace('/[^A-Za-z0-9\-]/', '', $string);
